...
IMPORTANT NOTICE: This investment involves significant risk and may result in the loss of the entire investment amount. The investor should consult with legal and financial advisors before executing this agreement.
  ...
  // Global Admin Settings API Endpoints

  // Global Fee Tiers Management
  app.get('/api/admin/fee-tiers', requireAdmin, async (req: any, res) => {
    try {
      const feeTiers = await db.select().from(globalFeeTiers).orderBy(globalFeeTiers.country, globalFeeTiers.minGoal);
      res.json(feeTiers);
    } catch (error) {
  ...
  app.post('/api/admin/fee-tiers', requireAdmin, async (req: any, res) => {
    try {
      const { country, currency, minGoal, maxGoal, feePercent, isDefault } = req.body;
      
      const [newFeeTier] = await db.insert(globalFeeTiers).values({
        country,
        currency,
        minGoal,
        maxGoal,
      ...
      await logAdminActivity(req.user.id, 'Fee Tier Management', `Created new fee tier for ${country}`);
      res.json(newFeeTier);
    } catch (error) {
  ...
  app.put('/api/admin/fee-tiers/:id', requireAdmin, async (req: any, res) => {
    try {
      const id = parseInt(req.params.id);
      const updates = req.body;
      
      const [updatedFeeTier] = await db
        .update(globalFeeTiers)
        .set(updates)
        .where(eq(globalFeeTiers.id, id))
        .returning();

      await logAdminActivity(req.user.id, 'Fee Tier Management', `Updated fee tier ID ${id}`);
      res.json(updatedFeeTier);
    } catch (error) {
  ...
  app.delete('/api/admin/fee-tiers/:id', requireAdmin, async (req: any, res) => {
    try {
      const id = parseInt(req.params.id);
      
      await db.delete(globalFeeTiers).where(eq(globalFeeTiers.id, id));
      await logAdminActivity(req.user.id, 'Fee Tier Management', `Deleted fee tier ID ${id}`);
      
      res.json({ message: "Fee tier deleted successfully" });
    } catch (error) {
  ...
  // KYC Policy Settings Management
  app.get('/api/admin/kyc-policies', requireAdmin, async (req: any, res) => {
    try {
      const kycPolicies = await db.select().from(kycPolicySettings).orderBy(kycPolicySettings.country, kycPolicySettings.minAmount);
      res.json(kycPolicies);
    } catch (error) {
      console.error("Error fetching KYC policies:", error);
      res.status(500).json({ message: "Failed to fetch KYC policies" });
    }
  });

  app.post('/api/admin/kyc-policies', requireAdmin, async (req: any, res) => {
    try {
      const { country, tier, minAmount, maxAmount, requirements } = req.body;
      
      const [newKycPolicy] = await db.insert(kycPolicySettings).values({
        country,
        tier,
        minAmount,
        maxAmount,
      ...
      await logAdminActivity(req.user.id, 'KYC Policy Management', `Created new KYC policy for ${country}`);
      res.json(newKycPolicy);
    } catch (error) {
      console.error("Error creating KYC policy:", error);
      res.status(500).json({ message: "Failed to create KYC policy" });
    }
  });

  app.put('/api/admin/kyc-policies/:id', requireAdmin, async (req: any, res) => {
    try {
      const id = parseInt(req.params.id);
      const updates = req.body;
      
      const [updatedKycPolicy] = await db
        .update(kycPolicySettings)
        .set(updates)
        .where(eq(kycPolicySettings.id, id))
        .returning();

      await logAdminActivity(req.user.id, 'KYC Policy Management', `Updated KYC policy ID ${id}`);
      res.json(updatedKycPolicy);
    } catch (error) {
      console.error("Error updating KYC policy:", error);
      res.status(500).json({ message: "Failed to update KYC policy" });
    }
  });

  // Region Settings Management
  app.get('/api/admin/region-settings', requireAdmin, async (req: any, res) => {
    try {
      const regionSettings = await db.select().from(regionSettings).orderBy(regionSettings.country);
      res.json(regionSettings);
    } catch (error) {
      console.error("Error fetching region settings:", error);
      res.status(500).json({ message: "Failed to fetch region settings" });
    }
  });

  app.post('/api/admin/region-settings', requireAdmin, async (req: any, res) => {
    try {
      const { country, currency, language, maxGoal, isEnabled, timezone } = req.body;
      
      const [newRegionSetting] = await db.insert(regionSettings).values({
        country,
        currency,
        language,
        maxGoal,
      ...
      await logAdminActivity(req.user.id, 'Region Settings Management', `Created new region setting for ${country}`);
      res.json(newRegionSetting);
    } catch (error) {
      console.error("Error creating region setting:", error);
      res.status(500).json({ message: "Failed to create region setting" });
    }
  });

  app.put('/api/admin/region-settings/:id', requireAdmin, async (req: any, res) => {
    try {
      const id = parseInt(req.params.id);
      const updates = req.body;
      
      const [updatedRegionSetting] = await db
        .update(regionSettings)
        .set(updates)
        .where(eq(regionSettings.id, id))
        .returning();

      await logAdminActivity(req.user.id, 'Region Settings Management', `Updated region setting ID ${id}`);
      res.json(updatedRegionSetting);
    } catch (error) {
      console.error("Error updating region setting:", error);
      res.status(500).json({ message: "Failed to update region setting" });
    }
  });
  ...
  app.get('/api/admin/payment-gateways', requireAdmin, async (req: any, res) => {
    try {
      const gateways = await db.select().from(paymentGateways).orderBy(paymentGateways.country, paymentGateways.currency);
      res.json(gateways);
    } catch (error) {
  ...
  app.post('/api/admin/payment-gateways', requireAdmin, async (req: any, res) => {
    try {
      const { country, currency, gateway, isDefault, isEnabled, settings } = req.body;
      ...
      await logAdminActivity(req.user.id, 'Payment Gateway Management', `Created new payment gateway for ${country}`);
      res.json(newGateway);
    } catch (error) {
  ...
  // Fee Simulation Tool
  app.post('/api/admin/simulate-fees', requireAdmin, async (req: any, res) => {
    try {
      const { campaignGoal, country, currency } = req.body;
      ...
      const feeTier = await db
        ...
        .where(
          and(
            eq(globalFeeTiers.country, country),
            eq(globalFeeTiers.currency, currency),
            gte(globalFeeTiers.maxGoal, campaignGoal),
            gte(campaignGoal, globalFeeTiers.minGoal)
          )
        )
        .limit(1);

      const platformFeePercent = feeTier[0]?.feePercent || 0;
      const platformFee = (parseFloat(campaignGoal) * parseFloat(platformFeePercent)) / 100;
      const processingFeePercent = 2.9; // Example processing fee
      const processingFee = (parseFloat(campaignGoal) * processingFeePercent) / 100;
      const totalFees = platformFee + processingFee;
      const netAmount = parseFloat(campaignGoal) - totalFees;

      res.json({
        campaignGoal: parseFloat(campaignGoal),
        platformFeePercent,
        platformFee,
      ...
      res.status(500).json({ message: "Failed to simulate fees" });
    }
  });

  // Legal Documents Management
  app.get('/api/admin/legal-documents', requireAdmin, async (req: any, res) => {
    try {
      const documents = await db.select().from(legalDocuments).orderBy(legalDocuments.country, legalDocuments.documentType);
      res.json(documents);
    } catch (error) {
  ...
  app.post('/api/admin/legal-documents', requireAdmin, async (req: any, res) => {
    try {
      const { country, documentType, title, content, url, version, isActive } = req.body;
      ...
      await logAdminActivity(req.user.id, 'Legal Document Management', `Created new legal document for ${country}`);
