...
IMPORTANT NOTICE: This investment involves significant risk and may result in the loss of the entire investment amount. The investor should consult with legal and financial advisors before executing this agreement.
  ...
  app.post('/api/founder/email-campaigns', async (req: any, res) => {
    try {
      ...
      if (currentUsage + recipients.length > 20) {
        return res.status(400).json({ 
          message: `Daily limit exceeded. You can send ${20 - currentUsage} more emails today.` 
        });
      }
        ...
        for (const email of emails) {
          try {
            const emailSent = await emailService.sendEmail({
              ...
              html: `
                <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333333; line-height: 1.6;">
                  ${email.personalizedMessage.replace(/\n/g, '<br><br>')}
                  
                  <br><br>
                  Best regards,<br>
                  ${emailSettings[0].displayName}
                  ${emailSettings[0].signature ? `<br><br>${emailSettings[0].signature.replace(/\n/g, '<br>')}` : ''}
                </div>
              `,
