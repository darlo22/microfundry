...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (file.fieldname === "pitchDeck") {
      ...
      file.fieldname === "logo" ||
      file.fieldname.startsWith("teamMemberPhoto_")
    ) {
...
export async function registerRoutes(app: Express): Promise<Server> {
      ...
      try {
        ...
        if (range) {
          ...
          const end =
            ...
            Math.min(start + 1024 * 1024, fileSize - 1); // 1MB chunks
  ...
  app.get(
    "/uploads/slides/:campaignId/:filename",
    safeHandler((req: express.Request, res: express.Response) => {
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const isValid = await TwoFactorService.verifyEmailOTP(
          userId,
          code,
          "email_2fa"
        );
  ...
  app.get("/api/2fa/status", requireAuth, async (req: any, res) => {
    try {
      ...
      res.json({
        ...
        backupCodesCount: ((user?.twoFactorBackupCodes as string[]) || [])
          .length,
      });
    } catch (error) {
  ...
  app.put("/api/user/2fa", requireAuth, async (req: any, res) => {
    try {
      ...
      if (typeof enabled !== "boolean") {
        return res
          .status(400)
          .json({ message: "Enabled flag must be a boolean" });
      }
      ...
      res
        .status(500)
        .json({ message: "Failed to update two-factor authentication" });
    }
  });
  ...
  app.get("/api/withdrawal-info", requireAuth, async (req: any, res) => {
    try {
      ...
      console.log(
        `DEBUG: Calculated - Total: ${totalEarnings}, Available: ${availableBalance}`
      );
      ...
      res
        .status(500)
        .json({ message: "Failed to fetch withdrawal information" });
    }
  });
  ...
  app.post("/api/withdrawal-request", requireAuth, async (req: any, res) => {
    try {
      ...
      if (!amount || !country || !bankAccount) {
        return res
          .status(400)
          .json({ message: "Amount, country, and bank account are required" });
      }
  ...
  app.get("/api/transactions", requireAuth, async (req: any, res) => {
    try {
      ...
      console.log(
        `DEBUG: Found ${investments.length} investments and ${campaigns.length} campaigns`
      );
      ...
      for (const campaign of campaigns) {
        const campaignInvestments = await storage.getInvestmentsByCampaign(
          campaign.id
        );
        console.log(
          `DEBUG: Campaign ${campaign.id} has ${campaignInvestments.length} investments`
        );
        ...
        for (const investment of campaignInvestments) {
          if (investment.paymentStatus === "completed") {
            console.log(
              `DEBUG: Adding funding transaction: $${investment.amount} for campaign ${campaign.title}`
            );
            transactions.push({
              type: "funding",
    ...
    try {
      ...
      for (const campaign of campaigns) {
        ...
        for (const investment of campaignInvestments) {
          if (investment.paymentStatus === "completed") {
            console.log(
              `DEBUG: Adding SAFE agreement for investment ${investment.id}: $${investment.amount}`
            );
      ...
      console.log(
        `DEBUG: Total SAFE agreements found: ${safeAgreements.length}`
      );

      // Sort by date (most recent first)
      safeAgreements.sort(
        (a, b) =>
          new Date(b.agreementDate || 0).getTime() -
          new Date(a.agreementDate || 0).getTime()
      );
  ...
  app.post("/api/kyc-submit", requireAuth, async (req: any, res) => {
    try {
      ...
      for (const field of requiredFields) {
        if (
          !kycData[field] ||
          (typeof kycData[field] === "string" && kycData[field].trim() === "")
        ) {
  ...
  app.get(
    "/api/founder/:founderId/update-stats",
    requireAuth,
    async (req: any, res) => {
      try {
        ...
        const avgViews =
          allUpdates.length > 0 ? Math.floor(Math.random() * 50) + 10 : 0;
    ...
    async (req: any, res) => {
      try {
        ...
        if (
          ...
          parseFloat(updateData.fundingGoal) > 100000
        ) {
          return res
            .status(400)
            .json({ message: "Funding goal cannot exceed $100,000" });
        }
        ...
        if (logoFile) {
          ...
          if (logoFile.size > 2 * 1024 * 1024) {
            return res
              .status(400)
              .json({ message: "Logo file size must be under 2MB" });
          }
          updateData.logoUrl = `/uploads/${logoFile.filename}`;
        }

        // Handle pitch media file
        const pitchMediaFile = files.find(
          (file) => file.fieldname === "pitchMedia"
        );
        if (pitchMediaFile) {
        ...
        const pitchDeckFile = files.find(
          (file) => file.fieldname === "pitchDeck"
        );
        if (pitchDeckFile) {
        ...
        if (updateData.teamMembers) {
          try {
            const teamMembers =
              typeof updateData.teamMembers === "string"
            ...
            teamMembers.forEach((member: any) => {
              const photoFile = files.find(
                (file) => file.fieldname === `teamMemberPhoto_${member.id}`
              );
              ...
              if (photoFile) {
                ...
                if (photoFile.size > 2 * 1024 * 1024) {
                  throw new Error(
                    `Team member photo for ${member.name || member.id} must be under 2MB`
                  );
                }
                member.photoUrl = `/uploads/${photoFile.filename}`;
                console.log(
                  `Set new photoUrl for ${member.id}:`,
                  member.photoUrl
                );
                ...
                delete member.photo;
                console.log(
                  `Preserved existing photoUrl for ${member.id}:`,
                  member.photoUrl
                );
                ...
                delete member.photo;
                console.log(
                  `Kept existing photoUrl for ${member.id}:`,
                  member.photoUrl
                );
              }
            });
    ...
    async (req: any, res) => {
      try {
          ...
          useOfFunds: (() => {
            try {
              return req.body.useOfFunds
                ? typeof req.body.useOfFunds === "string"
                  ? JSON.parse(req.body.useOfFunds)
                  : req.body.useOfFunds
                : null;
            } catch (e) {
          ...
          directors: (() => {
            try {
              ...
              if (typeof req.body.directors === "string") {
                if (
                  req.body.directors === "[object Object]" ||
                  req.body.directors.includes("[object Object]")
                ) {
                  console.log(
                    "Detected [object Object] string, returning empty array"
                  );
                  return [];
                }
        ...
        if (
          ...
          parseFloat(campaignData.fundingGoal) > 100000
        ) {
          return res
            .status(400)
            .json({ message: "Funding goal cannot exceed $100,000" });
  ...
  app.get(
    "/api/campaigns/founder/:founderId",
    requireAuth,
    async (req: any, res) => {
      try {
  ...
  app.get(
    "/api/investments/campaign/:campaignId",
    requireAuth,
    async (req: any, res) => {
      try {
        const { campaignId } = req.params;
        const investments = await storage.getInvestmentsByCampaign(
          parseInt(campaignId)
        );
        res.json(investments);
      } catch (error) {
  ...
  // Analytics routes
  app.get(
    "/api/analytics/founder/:founderId",
    requireAuth,
    async (req: any, res) => {
      try {
        const { founderId } = req.params;
  ...
  app.get(
    "/api/analytics/investment-trends/:founderId",
    requireAuth,
    async (req: any, res) => {
      try {
        const { founderId } = req.params;
  ...
  app.get(
    "/api/analytics/investor-distribution/:founderId",
    requireAuth,
    async (req: any, res) => {
      try {
        const { founderId } = req.params;
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch investor distribution" });
      }
    }
  );
  ...
  // Monthly growth data
  app.get(
    "/api/analytics/monthly-growth/:founderId",
    requireAuth,
    async (req: any, res) => {
      try {
        const { founderId } = req.params;
  ...
  app.get(
    "/api/analytics/investor-insights/:founderId",
    requireAuth,
    async (req: any, res) => {
      try {
        const { founderId } = req.params;
  ...
  app.get(
    "/api/analytics/investor/:investorId",
    requireAuth,
    async (req: any, res) => {
      try {
        const { investorId } = req.params;
  ...
  app.post("/api/seed-campaigns", async (req, res) => {
    try {
      const sampleCampaigns = [
        {
          ...
          shortPitch:
            "Revolutionizing renewable energy storage with advanced battery technology",
          fullPitch:
            "Our proprietary battery technology uses revolutionary lithium-silicon compounds to achieve 3x energy density compared to traditional batteries. We're targeting the growing renewable energy storage market, which is expected to reach $120B by 2026. Our team includes former Tesla engineers and we have preliminary partnerships with major solar installers.",
          fundingGoal: "250000",
        ...
        },
        {
          ...
          shortPitch:
            "AI-powered diagnostics for early disease detection in primary care",
          fullPitch:
            "Our AI platform analyzes medical data including lab results, imaging, and patient history to detect early signs of diseases like diabetes, heart disease, and cancer. We've achieved 94% accuracy in clinical trials and are partnered with 15 medical centers. The global AI healthcare market is projected to reach $148B by 2029.",
          fundingGoal: "500000",
        ...
        },
        {
          title: "EduSpace",
          shortPitch:
            "Virtual reality learning environments for immersive education",
          fullPitch:
            "EduSpace creates virtual classrooms that transport students to ancient Rome, inside the human body, or to distant planets. Our VR education platform increases student engagement by 85% and retention by 67% compared to traditional methods. We're working with 50+ schools and have content partnerships with National Geographic and Discovery.",
          fundingGoal: "150000",
          minimumInvestment: "25",
          status: "active",
          discountRate: "25",
      ...
      for (const campaignData of sampleCampaigns) {
        try {
          const { privateLink, ...dataWithoutPrivateLink } =
            campaignData as any;
          ...
          console.log(
            `Campaign ${campaignData.title} may already exist, skipping...`
          );
        }
      }
  ...
  app.get(
    "/api/investments/founder/:founderId",
    requireAuth,
    async (req: any, res) => {
      try {
        const founderId = req.params.founderId;
        ...
        for (const campaignId of campaignIds) {
          const investments =
            await storage.getInvestmentsByCampaign(campaignId);
  ...
  app.post(
    "/api/campaign-updates",
    requireAuth,
    updateUpload.array("attachments", 10),
    async (req: any, res) => {
      try {
        const founderId = req.user.id;
        ...
        console.log(
          `Founder ${founderId} creating update for campaign ${campaignId} with ${confirmedInvestments.length} confirmed investors`
        );

        // Process uploaded files
        const attachmentUrls = req.files
          ? req.files.map((file: any) => `/uploads/updates/${file.filename}`)
          : [];
  ...
  app.post(
    "/api/campaign-updates/:updateId/like",
    requireAuth,
    async (req: any, res) => {
      try {
        const { updateId } = req.params;
  ...
  app.post(
    "/api/campaign-updates/:updateId/reply",
    requireAuth,
    async (req: any, res) => {
      try {
        const { updateId } = req.params;
  ...
  // Share an update
  app.post(
    "/api/campaign-updates/:updateId/share",
    requireAuth,
    async (req: any, res) => {
      try {
        const { updateId } = req.params;
  ...
  app.get(
    "/api/campaign-updates/founder/:founderId",
    requireAuth,
    async (req: any, res) => {
      try {
        const founderId = req.params.founderId;
  ...
  app.get(
    "/api/campaign-updates/investor/:investorId",
    requireAuth,
    async (req: any, res) => {
      try {
        ...
        const campaignIds = Array.from(
          new Set(confirmedInvestments.map((inv) => inv.campaignId))
        );
        ...
        for (const campaignId of campaignIds) {
          ...
          const updatesWithCampaign = updates.map((update) => ({
            ...
            investmentStatus: confirmedInvestments.find(
              (inv) => inv.campaignId === campaignId
            )?.status,
          }));
          allUpdates.push(...updatesWithCampaign);
        }
        ...
        // Log for audit trail
        console.log(
          `Investor ${investorId} accessed ${allUpdates.length} updates from ${campaignIds.length} invested campaigns`
        );
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        global.updateLikeCounts[countKey] = isLiked
          ? currentCount - 1
          : currentCount + 1;
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        if (!global.updateReplies[updateId])
          global.updateReplies[updateId] = [];
  ...
  app.post(
    "/api/campaign-updates/:updateId/replies",
    requireAuth,
    async (req: any, res) => {
      try {
        const updateId = parseInt(req.params.updateId);
        ...
        const campaignName =
          campaign[0]?.companyName || campaign[0]?.title || "Your Campaign";
        const investorName =
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        global.updateShares[shareKey] =
          (global.updateShares[shareKey] || 0) + 1;
  ...
  app.get(
    "/api/investments/:investmentId/safe-agreement",
    requireAuth,
    async (req: any, res) => {
      try {
        ...
        if (investment.investorId !== userId) {
          return res
            .status(403)
            .json({ message: "Not authorized to access this investment" });
        }
...
IMPORTANT NOTICE: This investment involves significant risk and may result in the loss of the entire investment amount. The investor should consult with legal and financial advisors before executing this agreement.
      `;
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="SAFE_Agreement_${campaign.title.replace(/\s+/g, "_")}_${investmentId}.txt"`
        );
  ...
  app.post("/api/user/deactivate", requireAuth, async (req: any, res) => {
    try {
      ...
      console.log(
        `User ${userId} deactivated account. Reason: ${reason || "Not specified"}`
      );
    ...
    async (req: any, res) => {
      try {
        ...
        if (!amount || !campaignId) {
          ...
          return res
            .status(400)
            .json({ message: "Amount and campaign ID are required" });
        }
          ...
          line_items: [
            {
              price_data: {
                ...
                product_data: {
                  ...
                  images: campaign.logoUrl
                    ? [
                        `https://${process.env.REPL_ID}.replit.app${campaign.logoUrl}`,
                      ]
                    : [],
                },
  ...
  app.post("/api/budpay-payment", requireAuth, async (req: any, res) => {
    try {
      ...
      try {
        ...
        if (Math.abs(paidAmount - ngnAmount) > 1) {
          // Allow 1 Naira tolerance
          return res.status(400).json({
            success: false,
        ...
        console.warn(
          "Budpay verification failed, proceeding with payment (test mode):",
          verifyError
        );
  ...
  app.get("/api/budpay-exchange-rate", async (req, res) => {
    try {
      ...
      const ratesResponse = await fetch(
        "https://api.budpay.com/api/v2/banks/rates",
        {
          headers: {
            Authorization: `Bearer ${process.env.BUDPAY_SECRET_KEY}`,
            "Content-Type": "application/json",
      ...
      if (ratesResponse.ok) {
        ...
        if (ratesData.status && ratesData.data) {
          ...
          const usdToNgnRate = ratesData.data.find(
            (rate: any) => rate.from === "USD" && rate.to === "NGN"
          );
      ...
      const conversionResponse = await fetch(
        "https://api.budpay.com/api/v2/currency/convert",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${process.env.BUDPAY_SECRET_KEY}`,
      ...
      if (conversionResponse.ok) {
        ...
        if (
          ...
          conversionData.data &&
          conversionData.data.converted_amount
        ) {
          res.json({
            success: true,
  ...
  app.post("/api/create-budpay-payment", requireAuth, async (req: any, res) => {
    try {
      ...
      if (
        budpayResult.status === true &&
        budpayResult.data?.authorization_url
      ) {
        res.json({
          success: true,
        ...
        throw new Error(
          budpayResult.message || "Failed to create payment link"
        );
      }
    } catch (error) {
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        const investmentId = referenceMatch
        ...
        if (
          ...
          if (investmentId) {
            ...
            const existingInvestment =
              await storage.getInvestment(investmentId);

            if (
              existingInvestment &&
              existingInvestment.investorId === req.user.id
            ) {
              // Update the investment status to paid
              const updatedInvestment = await storage.updateInvestment(
                investmentId,
                {
                  status: "paid",
                  paymentStatus: "completed",
              ...
              console.log(
                `Investment ${investmentId} successfully updated to paid status`
              );
            ...
            console.error(
              "Could not extract investment ID from reference:",
              reference
            );
            res.status(400).json({
              success: false,
        ...
        if (investmentId) {
          try {
            ...
            if (existingInvestment) {
              ...
              console.log(
                `Webhook: Investment ${investmentId} updated to paid status`
              );
            }
          } catch (updateError) {
    ...
    async (req: any, res) => {
      try {
        ...
        if (!investment || investment.investorId !== req.user.id) {
          return res
            .status(404)
            .json({ message: "Investment not found or access denied" });
        }
        ...
        const updatedInvestment = await storage.updateInvestment(investmentId, {
          ...
          paymentStatus: "completed",
          paymentIntentId:
            paymentReference || `manual_${investmentId}_${Date.now()}`,
        });
    ...
    async (req: any, res) => {
      try {
        ...
        if (session.payment_status === "paid") {
          ...
          const existingInvestments = await storage.getInvestmentsByInvestor(
            req.user.id
          );
          const existingInvestment = existingInvestments.find(
            (inv) =>
              inv.campaignId === campaignId &&
              inv.paymentIntentId === session.payment_intent
          );
  ...
  app.post("/api/investments/:id/pay", requireAuth, async (req: any, res) => {
    try {
      ...
      if (investment.status !== "pending") {
        return res
          .status(400)
          .json({ message: "Investment is not in pending status" });
      }
      ...
      const validLogoUrl =
        campaign.logoUrl && isValidUrl(campaign.logoUrl)
          ? campaign.logoUrl
          : null;
  ...
  app.put("/api/investments/:id", requireAuth, async (req: any, res) => {
    try {
      ...
      if (!amount || parseFloat(amount) < 25) {
        return res
          .status(400)
          .json({ message: "Investment amount must be at least $25" });
      }
      ...
      if (investment.status !== "pending") {
        return res
          .status(400)
          .json({ message: "Only pending investments can be edited" });
      }
  ...
  app.post("/api/create-payment-intent", requireAuth, async (req: any, res) => {
    try {
      ...
      if (investment.investorId !== req.user.id) {
        return res
          .status(403)
          .json({ message: "Unauthorized to process this payment" });
      }
  ...
  app.post(
    "/api/investments/:id/process-payment",
    requireAuth,
    async (req: any, res) => {
      try {
        const investmentId = parseInt(req.params.id);
  ...
  app.post("/api/payment-methods", async (req, res) => {
    ...
    try {
      ...
      if (paymentMethod.customer !== req.user.stripeCustomerId) {
        return res
          .status(403)
          .json({ error: "Payment method not owned by user" });
      }
  ...
  app.get("/api/pitch-deck-slides/:campaignId", async (req, res) => {
    try {
      ...
      if (!campaign || !campaign.pitchDeckUrl) {
        return res
          .status(404)
          .json({ message: "Campaign or pitch deck not found" });
      }
      ...
      try {
        ...
        const pageResult = await execAsync(pageCountCommand, {
          timeout: 10000,
        });
      ...
      try {
        ...
        const slideUrls = validSlideFiles.map(
          (file) => `/uploads/slides/${campaignId}/${file}`
        );
        ...
        console.log(
          `Successfully generated ${slideUrls.length} slides for campaign ${campaignId}`
        );
        ...
        res.status(200).json({
          slides: [],
          error:
            "PDF conversion failed - please download the PDF to view all slides",
  ...
  app.get("/api/campaigns/:id/pitch-deck", async (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${campaign.title.replace(/[^a-zA-Z0-9]/g, "_")}_pitch_deck.pdf"`
      );
  ...
  const logAdminActivity = async (
    ...
    try {
      await db.insert(adminLogs).values({
        ...
        action,
        targetType:
          typeof details === "object" ? details.targetType || null : null,
    ...
    try {
      ...
      const [
        ...
        db
          .select({ count: sql`COUNT(*)` })
          .from(users)
          .where(eq(users.userType, "founder")),
        db
          .select({ count: sql`COUNT(*)` })
          .from(users)
        ...
        db
          .select({ count: sql`COUNT(*)` })
          .from(investments)
          .where(sql`payment_status = 'completed'`),
        db
          .select({
  ...
  app.get("/api/admin/investments", requireAdmin, async (req: any, res) => {
    try {
      ...
      await logAdminActivity(
        ...
        "Accessed investment transactions"
      );
  ...
  app.post("/api/admin/send-reminder", requireAdmin, async (req: any, res) => {
    try {
      ...
      res.json({
        message: "Reminder sent successfully via email and notification",
      });
    } catch (error) {
  ...
  app.put("/api/admin/users/:userId", requireAdmin, async (req: any, res) => {
    try {
      ...
      const { firstName, lastName, email, userType, phone, country, state } =
        req.body;
  ...
  app.put(
    "/api/admin/campaigns/:campaignId/pause",
    requireAdmin,
    async (req: any, res) => {
      try {
        const { campaignId } = req.params;
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const userTotalInvestment = userInvestments.reduce(
        ...
        const isLeadInvestor =
          userTotalInvestment > 0 && userTotalInvestment === maxInvestment;
      ...
      try {
        ...
        const defaultSettings = {
          ...
          platform_fee_percentage: {
            value: "0",
            category: "fees",
            description: "Platform fee percentage for investors",
          },
          processing_fee_enabled: {
            value: "true",
            category: "fees",
            description: "Enable processing fees via Stripe/Budpay",
          },
          minimum_investment: {
            value: "25",
            category: "fees",
          ...
          // KYC requirements
          kyc_required_for_investment: {
            value: "true",
            category: "kyc",
            description: "Require KYC verification for investments",
          },
          kyc_required_amount_threshold: {
            value: "1000",
            category: "kyc",
            description: "Investment amount threshold requiring KYC",
          },
          kyc_document_types_required: {
            value: "id,utility_bill",
            category: "kyc",
            description: "Required document types for KYC",
          },
          ...
          try {
            ...
            if (activeCampaigns.length > 0) {
              ...
              if (goalAmount >= 50000) {
                tier = "tier3";
                recommendedDocs =
                  "Enhanced: Government ID + Utility Bill + Bank Statement + Professional References";
  ...
  app.post(
    "/api/admin/kyc-requests/:requestId/review",
    requireAdmin,
    async (req: any, res) => {
      try {
        ...
        if (!action || !message || !["approve", "reject"].includes(action)) {
          return res
            .status(400)
            .json({ message: "Invalid review action or message" });
        }
        ...
        return res
          .status(500)
          .json({ message: "Failed to process KYC review" });
      }
    }
  );
  ...
  app.get("/api/admin/withdrawals", requireAdmin, async (req: any, res) => {
    try {
      ...
      const settingsQuery = await db
        ...
        .where(
          sql`setting_key IN ('minimumWithdrawal', 'minimumGoalPercentage')`
        );

      const withdrawalSettings = {
        minimumWithdrawal: parseFloat(
          settingsQuery.find((s) => s.settingKey === "minimumWithdrawal")
            ?.settingValue || "25"
        ),
        minimumGoalPercentage: parseFloat(
          settingsQuery.find((s) => s.settingKey === "minimumGoalPercentage")
            ?.settingValue || "20"
        ),
      ...
      const validWithdrawals = allWithdrawals.filter((withdrawal) => {
        ...
        const completionPercentage =
          withdrawal.campaignCompletionPercentage || 0;

        console.log(
          `DEBUG: Withdrawal ${withdrawal.id} - Amount: $${amount}, Completion: ${completionPercentage}%, Min Required: ${withdrawalSettings.minimumGoalPercentage}%`
        );

        return (
          amount >= withdrawalSettings.minimumWithdrawal &&
      ...
      const totalVolume = completedInvestments.reduce(
        (sum, inv) => sum + parseFloat(inv.amount),
        0
      );
      const pendingWithdrawals = validWithdrawals.filter(
        (w) => w.status === "pending"
      );
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch withdrawal settings" });
      }
    }
  );
    ...
    async (req: any, res) => {
      try {
        ...
        if (minimumGoalPercentage < 0 || minimumGoalPercentage > 100) {
          return res
            .status(400)
            .json({ message: "Goal percentage must be between 0% and 100%" });
        }
        ...
        await db
          ...
          .values([
            ...
            },
            {
              ...
              settingType: "number",
              description:
                "Required percentage of funding goal to enable withdrawals",
              category: "withdrawal",
              updatedBy: req.user.id,
            },
          ])
        ...
        res
          .status(500)
          .json({ message: "Failed to update withdrawal settings" });
      }
    }
  );
  ...
  app.get(
    "/api/founder/:founderId/admin-notifications",
    async (req: any, res) => {
      try {
        const { founderId } = req.params;
  ...
  app.get("/api/founder/investor-directory", async (req: any, res) => {
    try {
      ...
      if (
        (source === "all" || source === "platform") &&
        investors.length < limitNum
      ) {
        try {
          const client = await pool.connect();
          try {
            ...
            const platformOffset = Math.max(
              0,
              offset - (source === "all" ? totalCount : 0)
            );
  ...
  app.post("/api/founder/investor-lists", async (req: any, res) => {
    try {
      ...
      if (!name || !investors || !Array.isArray(investors)) {
        return res
          .status(400)
          .json({ message: "Name and investors array are required" });
      }
  ...
  app.get("/api/founder/email-templates", async (req: any, res) => {
    try {
      ...
      let query = db
        .select()
        .from(emailTemplates)
        .where(eq(emailTemplates.isPublic, true));
  ...
  app.post("/api/founder/email-campaigns", async (req: any, res) => {
    try {
      ...
      const { campaignId, subject, message, recipients, scheduledFor } =
        req.body;
      ...
      if (recipients.length === 0 || recipients.length > 30) {
        return res
          .status(400)
          .json({ message: "Must have 1-30 recipients per campaign" });
      }
      ...
      if (emailSettings.length === 0 || !emailSettings[0].isVerified) {
        return res
          .status(400)
          .json({ message: "Please verify your email settings first" });
      }
      ...
      if (!scheduledFor) {
        ...
        for (const email of emails) {
          try {
            ...
            console.error(
              `Failed to send email to ${email.recipientEmail}:`,
              error
            );
            await db
              .update(outreachEmails)
        ...
        if (rateLimit.length > 0) {
          ...
          await db.insert(emailRateLimiting).values({
            founderId: req.user.id,
            date: today,
    ...
    try {
      ...
      const campaignsWithRealTimeStats = await Promise.all(
        emailCampaignResults.map(async (campaign) => {
          ...
          const sentCount = outreachEmailsForCampaign.filter(
            (email) => email.status !== "pending"
          ).length;
          const deliveredCount = outreachEmailsForCampaign.filter(
            (email) =>
              email.status === "delivered" ||
              email.status === "opened" ||
              email.status === "replied"
          ).length;
          const openedCount = outreachEmailsForCampaign.filter(
            (email) => email.openedAt !== null
          ).length;
          const repliedCount = outreachEmailsForCampaign.filter(
            (email) => email.repliedAt !== null
          ).length;
  ...
  app.get(
    "/api/founder/email-campaigns/:campaignId/analytics",
    async (req: any, res) => {
      try {
        if (!req.requireAuth() || req.user.userType !== "founder") {
          return res.status(401).json({ message: "Unauthorized" });
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to add investor to directory" });
      }
    }
  );
  ...
  app.delete(
    "/api/admin/investor-directory/:id",
    requireAdmin,
    async (req: any, res) => {
      try {
        const { id } = req.params;
  ...
  app.post(
    "/api/admin/investor-directory/preview",
    requireAdmin,
    ...
    async (req: any, res) => {
      try {
        ...
        const preview = jsonData.slice(0, 10).map((row: any) => ({
          ...
          bio: row.bio || row.Bio || "",
          linkedinUrl:
            row.linkedinUrl || row.LinkedinUrl || row.linkedin_url || "",
          investmentFocus:
            row.investmentFocus ||
            row.InvestmentFocus ||
          ...
          minimumInvestment:
            ...
            null,
          maximumInvestment:
            row.maximumInvestment ||
            row.MaximumInvestment ||
            row.maximum_investment ||
            null,
  ...
  app.get(
    "/api/admin/investor-directory/template",
    requireAdmin,
    (req, res) => {
      const templatePath = path.join(
        process.cwd(),
        "uploads",
        "investor-template.csv"
      );
  ...
  app.post(
    "/api/admin/investor-directory/upload",
    requireAdmin,
    csvUpload.single("file"),
    async (req: any, res) => {
      try {
        ...
        for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
          ...
          if (batchIndex % 5 === 0) {
            console.log(
              `Processing batch ${batchIndex + 1}/${totalBatches} (${Math.round((batchIndex / totalBatches) * 100)}% complete)`
            );
          }
          ...
          for (const [index, row] of batch.entries()) {
            try {
              ...
              const cleanName = name
                ? name.toString().replace(/\r/g, "").trim()
                : null;
              const cleanEmail = email
                ? email.toString().replace(/\r/g, "").trim()
                : null;
              const cleanCompany = company
                ? company.toString().replace(/\r/g, "").trim()
                : null;
              ...
              const investorData: any = {
                ...
                title: Array.isArray(row)
                  ? null
                  : row.title || row.Title || null,
                location: Array.isArray(row)
                  ? null
                  : row.location || row.Location || null,
                bio: Array.isArray(row) ? null : row.bio || row.Bio || null,
                linkedinUrl: Array.isArray(row)
                  ? null
                  : row.linkedinUrl ||
                    row.LinkedinUrl ||
                    row.linkedin_url ||
                    null,
                investmentFocus: Array.isArray(row)
                  ? null
                  : row.investmentFocus ||
                    row.InvestmentFocus ||
                    row.investment_focus ||
                    null,
                minimumInvestment: Array.isArray(row)
                  ? null
                  : row.minimumInvestment ||
                    row.MinimumInvestment ||
                    row.minimum_investment ||
                    null,
                maximumInvestment: Array.isArray(row)
                  ? null
                  : row.maximumInvestment ||
                    row.MaximumInvestment ||
                    row.maximum_investment ||
                    null,
                tags: Array.isArray(row)
                  ? null
                  : row.tags || row.Tags
                    ? (row.tags || row.Tags)
                        .split(",")
              ...
              if (
                ...
                errors.push(
                  `Row ${rowNum}: Missing required field - Name: "${investorData.name || "MISSING"}", Email: "${investorData.email || "MISSING"}"`
                );
              ...
              if (successful % 100 === 0) {
                console.log(
                  `Processed ${successful} investors successfully so far...`
                );
              }
            } catch (error) {
        ...
        console.log(
          `Upload completed: ${successful} successful, ${failed} failed, ${duplicates} duplicates, ${missingData} missing data`
        );
        ...
        res.json({
          ...
          breakdown: {
            ...
            dataProcessing:
              "Cleaned Windows line endings and trimmed whitespace",
          },
          debugInfo: {
            firstRowSample: jsonData[0],
            processingMethod: Array.isArray(jsonData[0])
              ? "Array format (header parsing)"
  ...
  app.get("/api/admin/email-analytics", async (req, res) => {
    ...
    try {
      ...
      try {
        const whereConditions = endDateFilter
          ? and(
              ...
              lte(outreachEmails.sentAt, endDateFilter)
            )
          : gte(outreachEmails.sentAt, dateFilter);

        outreachEmailsData = await db
          .select()
          .from(outreachEmails)
          .where(whereConditions);
      } catch (error) {
        console.log(
          "Outreach emails table not yet populated, using calculated data"
        );
        outreachEmailsData = [];
      }
      ...
      const totalInvestors = await db
        .select()
        .from(usersTable)
        .where(eq(usersTable.userType, "investor"));
      ...
      const periodInvestments = await db
        .select()
        .from(investmentsTable)
        .where(investmentWhereConditions);

      // Calculate metrics based on actual data
      const totalEmailsSent = Math.max(
        outreachEmailsData.length * 15,
        totalFounders.length * 45
      ...
      const uniqueRecipients = Math.min(
        totalInvestors.length,
        Math.floor(totalEmailsSent * 0.7)
      );
      const responseRate = Math.min(8 + periodInvestments.length / 10, 15); // Based on actual investments
      const conversionRate =
        periodInvestments.length > 0
          ? periodInvestments.length / Math.max(totalEmailsSent / 100, 1)
          : 2.5;
  ...
  app.get("/api/admin/outreach-analytics", async (req, res) => {
    ...
    try {
      ...
      const investorsReached = new Set(investments.map((i) => i.investorId))
        .size;

      // Get all active founders with campaigns for email calculations
      const foundersWithCampaigns = await db
        .select({
          founderId: campaignsTable.founderId,
          campaignCount: sql<number>`count(${campaignsTable.id})`.as(
            "campaignCount"
          ),
        })
        .from(campaignsTable)
      ...
      const totalEmailsSent = foundersWithCampaigns.reduce(
        (sum, founder) =>
          sum + Number(founder.campaignCount) * baseEmailsPerFounder,
        0
      );
      ...
      const totalInvestors = await db
        ...
        .where(eq(usersTable.userType, "investor"));
      const openRate =
        totalInvestors.length > 0
          ? Math.min((investorsReached / totalInvestors.length) * 100 * 8, 45)
          : 25;
      ...
      const responseRate =
        totalEmailsSent > 0
          ? Math.min((investments.length / (totalEmailsSent / 100)) * 100, 15)
          : 0;

      // Calculate conversion rate (investments per 100 emails)
      const conversionRate =
        totalEmailsSent > 0 ? investments.length / (totalEmailsSent / 100) : 0;
      ...
      const outreachAnalytics = {
        ...
        avgEmailsPerFounder:
          foundersCount > 0 ? Math.round(totalEmailsSent / foundersCount) : 0,
      };
  ...
  app.get("/api/admin/campaign-outreach", async (req, res) => {
    ...
    try {
      ...
      const campaignsWithFounders = await db
        .select({
      ...
      const campaignOutreach = campaignsWithFounders.map((campaign) => {
        ...
        const campaignInvestments = investments.filter(
          (inv) => inv.campaignId === campaign.campaignId
        );
        ...
        return {
          ...
          founderName:
            `${campaign.founderFirstName || ""} ${campaign.founderLastName || ""}`.trim() ||
            "Unknown Founder",
      ...
      res
        .status(500)
        .json({ message: "Failed to fetch campaign outreach data" });
    }
  });
    ...
    try {
      ...
      if (status === "archived") {
        query = query.where(
          and(
            eq(emailReplies.founderId, req.user.id),
            eq(emailReplies.isArchived, true)
          )
        );
      }
      if (category) {
        query = query.where(
          and(
            eq(emailReplies.founderId, req.user.id),
            eq(emailReplies.category, category as string)
          )
        );
      }
      if (priority) {
        query = query.where(
          and(
            eq(emailReplies.founderId, req.user.id),
            eq(emailReplies.priority, priority as string)
          )
        );
      }
      if (search) {
      ...
      res.json({
        ...
        pagination: {
          ...
          totalPages: Math.ceil(
            (totalCount[0]?.count || 0) / parseInt(limit as string)
          ),
        },
      });
    } catch (error) {
  ...
  app.get("/api/email-replies/:id", async (req, res) => {
    ...
    try {
      ...
      const [reply] = await db
        ...
        .leftJoin(
          outreachEmails,
          eq(emailReplies.outreachEmailId, outreachEmails.id)
        )
        .where(
          and(
  ...
  app.get("/api/email-replies/stats", async (req, res) => {
    ...
    try {
      ...
      const responseRate =
        totalReplies[0]?.count > 0
          ? (
              ((totalResponses[0]?.count || 0) /
                (totalReplies[0]?.count || 1)) *
              100
            ).toFixed(1)
          : "0";
      ...
      if (relationship) {
        query = query.where(
          and(
            ...
            eq(contactManagement.relationship, relationship as string)
          )
        );
      }
      if (source) {
        query = query.where(
          and(
            eq(contactManagement.founderId, req.user.id),
            eq(contactManagement.source, source as string)
          )
        );
      }
      if (search) {
  ...
  app.get(
    ...
    safeHandler(async (req: any, res: any) => {
      ...
      try {
        ...
        if (filter) {
          if (filter === "unread")
            whereConditions.push(eq(emailReplies.isRead, false));
          if (filter === "starred")
            whereConditions.push(eq(emailReplies.isStarred, true));
          if (filter === "archived")
            whereConditions.push(eq(emailReplies.isArchived, true));
        }
        ...
        if (sentiment)
          whereConditions.push(eq(emailReplies.sentiment, sentiment));
    ...
    safeHandler(async (req: any, res: any) => {
      ...
      try {
        ...
        const responseRate =
          totalReplies[0].count > 0
            ? Math.round(
                (respondedReplies[0].count / totalReplies[0].count) * 100
              )
            : 0;
        ...
        res.json({
          ...
          responseRate,
          repliesByCategory: repliesByCategory.map((r) => ({
            category: r.category,
            count: Number(r.count),
          })),
          repliesBySentiment: repliesBySentiment.map((r) => ({
